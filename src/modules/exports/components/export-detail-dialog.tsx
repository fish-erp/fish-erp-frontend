...
export function ExportDetailDialog({ invoice: initial, open, onOpenChange }: { invoice: ExportInvoice | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-foreground/40" /><Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[92vh] w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border bg-white p-6 shadow-2xl">
    ...
    {detail.isError ? <p role="alert" className="mt-4 text-danger">Không tải được thanh toán mới nhất. <button onClick={() => void detail.refetch()}>Thử lại</button></p> : detail.data ? <PaymentPanel key={invoice.id} invoice={invoice} /> : <p className="mt-4 text-sm">Đang tải thanh toán…</p>}
