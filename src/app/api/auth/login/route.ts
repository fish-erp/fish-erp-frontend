...
export async function POST(request: Request) {
  const input = await request.json() as { identifier?: string; password?: string; remember?: boolean };
  const upstream = await fetch(backendUrl("auth/login"), { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ identifier: input.identifier, password: input.password }), cache: "no-store" });
